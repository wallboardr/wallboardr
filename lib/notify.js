...
var boardListeners = {},
    addBoardListener = function (board, spark) {
      ...
      console.log('Adding listener for board: ' + board);
    },
      ...
      if (board) {
        for (; lindex < boardListeners[board].length; lindex += 1) {
          if (spark.id === boardListeners[board][lindex].id) {
            ...
            console.log('Removing listener for board: ' + board);
            break;
          }
        }
      }
    ...
    notifyBoardListeners = function (board) {
      ...
      if (board && boardListeners[board]) {
        ...
        console.log('Notifying listeners about update to board: ' + board);
      }
    };
  ...
  listen: function (server) {
    ...
    primus.on('connection', function (spark) {
      if (spark.query && spark.query.board) {
        ...
        console.log('Admin connection, waiting for updates to boards...');
