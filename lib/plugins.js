var mustache = require('mustache'),
    ...
    createViewData = function (plugins, type) {
      return {plugins: plugins.map(function (name) {
    ...
    processPlugin = function (plugins, jsTemplate, lessTemplate, type, lessType) {
      var viewData = createViewData(plugins, type),
    ...
    createPluginFiles = function (plugins) {
      var jsTemplate = readTemplate('./plugin-list.template.js'),
          lessTemplate = readTemplate('./plugins.template.less');
      processPlugin(plugins, jsTemplate, lessTemplate, 'admin', 'app');
      processPlugin(plugins, jsTemplate, lessTemplate, 'screen', 'display');
    }
    load = function (file) {
      ...
      if (plugins.loadPlugins && plugins.loadPlugins.length) {
        ...
        createPluginFiles(plugins.loadPlugins);
      }
    };
